...
# AI Configuration
api_key_openai = os.getenv("OPENAI_API_KEY_1")
...
def create_openai_embedding(input_text):
    try:
...
# Initialize embeddings and LLM with the custom base URL
...
llm = ChatGoogleGenerativeAI(model="gemini-2.0-flash-exp", google_api_key=api_key_cycle, temperature=0.5)
...
def process_pdf_data(file_path, file_name):
    ...
    try:
        ...
        embeddings_list = [create_openai_embedding(chunk.page_content) for chunk in chunks]
        if None in embeddings_list:
            raise ValueError("Error generating embeddings for one or more chunks.")

        # Save the chunks to ChromaDB
        vectorstore = Chroma.from_documents(
            documents=chunks,
            embedding=openai_embedding_function,
...
def get_llm_response(question, col_name):
    vectorstore = Chroma(persist_directory=PERSIST_DIR, collection_name=col_name, embedding_function=openai_embedding_function)
    data = vectorstore.similarity_search(question, k=8)
